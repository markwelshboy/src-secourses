...
def convert_image_for_jpeg(image):
    """
    Convert an image to RGB mode if it has transparency (RGBA/LA/P modes with transparency).
    This ensures the image can be saved as JPEG format.
    ...
    """
    # If image is already RGB or grayscale (L), return as is
    if image.mode == 'RGB':
        return image
    
    # For RGBA, LA, or palette modes with transparency, convert to RGB
    if image.mode in ('RGBA', 'LA', 'P'):
        # Create a white background
        if image.mode == 'P':
            # Convert palette to RGBA first to handle transparency
    ...
    # For other modes (L, CMYK, etc.), convert to RGB
        ...
        try:
            ...
            if not save_as_png:
                ...
                with Image.open(img_path) as original_pil:
                    # Convert to RGB and handle transparency using helper function
                    pil_img = convert_image_for_jpeg(original_pil)
            ...
            for target_width, target_height in parsed_aspect_ratios:
                ...
                try:
                    if save_as_png:
                        ...
                        cropped_img = convert_image_for_jpeg(cropped_img)
            ...
            if save_yolo and yolo_folder:
                ...
                try:
                    if sam2_prompt:
                        if save_as_png:
                            segmented_pil.save(yolo_save_path, format='PNG')
                        else:
                            # Convert RGBA to RGB for JPEG compatibility
                            segmented_pil = convert_image_for_jpeg(segmented_pil)
                            segmented_pil.save(yolo_save_path, format='JPEG', quality=100)
                    else:
                        result_img = Image.fromarray(results[0].plot())
                        if save_as_png:
                            result_img.save(yolo_save_path, format='PNG')
                        else:
                            # Convert RGBA to RGB for JPEG compatibility
                            result_img = convert_image_for_jpeg(result_img)
                            result_img.save(yolo_save_path, format='JPEG', quality=100)
                    processed += 1
                except Exception as e:
    ...
    try:
        ...
        for resolution in resolutions:
            ...
            if save_as_png:
                ...
                # Convert RGBA to RGB for JPEG compatibility
                resized_image = convert_image_for_jpeg(resized_image)
            ...
            for i in range(grid_dim):
                for j in range(grid_dim):
                    ...
                    if save_as_png:
                        ...
                        # Convert RGBA to RGB for JPEG compatibility (safety check)
                        tile = convert_image_for_jpeg(tile)
...
def process_image_face_enhanced(args):
    ...
    try:
        ...
        if save_as_png:
            ...
            # Convert RGBA to RGB for JPEG compatibility
            face_pil = convert_image_for_jpeg(face_pil)
...
def main():
    parser = argparse.ArgumentParser(description="Ultimate Image Processing Toolkit by SECourses")
    ...
    try:
        ...
        ctx = multiprocessing.get_context('spawn')
        globals()['Process'] = ctx.Process
        globals()['Queue'] = ctx.Queue
        globals()['Value'] = ctx.Value
        globals()['Lock'] = ctx.Lock
        globals()['Manager'] = ctx.Manager

    with gr.Blocks(title="Ultimate Image Processing Toolkit by SECourses") as demo:
        gr.Markdown("# Ultimate Image Processing Toolkit by SECourses V25 > https://www.patreon.com/posts/120352012")

        with gr.Tabs():
            with gr.Tab("Image Cropper"):
                gr.Markdown("""
                    ## Image Cropper
                    - Crop images using YOLO object detection or SAM2 segmentation
                    - Supports multi-GPU processing
                    - Enables batch processing for both YOLO and SAM2
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        output_folder = gr.Textbox(
                            label="Output Folder",
                            placeholder="Path for cropped images output"
                        )
                        aspect_ratios = gr.Textbox(
                            label="Aspect Ratios",
                            placeholder="e.g., 1024x1024,1280x720,1920x1080,...",
                            value="1024x1024"
                        )
                        yolo_folder = gr.Textbox(
                            label="YOLO/SAM2 Detected Images Folder (Optional)",
                            placeholder="Path for detected images output (optional)"
                        )
            
                        with gr.Row():
                            save_yolo = gr.Checkbox(
                                label="Save YOLO/SAM2 Detected Images",
                                value=False
                            )
                            save_as_png = gr.Checkbox(
                                label="Save as PNG (uncheck for JPG)",
                                value=True
                            )
                            overwrite_crop = gr.Checkbox(
                                label="Overwrite existing files",
                                value=False
                            )
                            skip_no_detection = gr.Checkbox(
                                label="Skip images with no SAM2 detection",
                                value=False
                            )
                        ...
                        with gr.Row():
                            selected_class = gr.Dropdown(
                                ...
                                label="Select Detection Class (YOLO)",
                                value="person",
                                interactive=True
                            )
                            sam2_prompt = gr.Textbox(
                                label="SAM2 Prompt (Leave empty to use YOLO)",
                                placeholder="e.g.: person. face. (end with periods)",
                                interactive=True
                            )
                        ...
                        with gr.Row():
                            ...
                            padding_unit = gr.Dropdown(
                                choices=["percent", "pixel"],
                                value="percent",
                                label="Padding Unit",
                                interactive=True
                            )
                    with gr.Column(scale=1):
                        status_display = gr.HTML(
                            ...
                            value="<div>Ready to start processing...</div>"
                        )
                        gr.HTML("<div>Progress:</div>")
                        ...
                        clear_button = gr.Button("Clear Status")

                def update_ui_status(msg):
                    if msg.startswith("ERROR:"):
                        return {
                            error_display: gr.update(value=msg[6:], visible=True),
                            status_display: gr.update(value="<div style='color: red;'>Error occurred</div>")
                        }
                    elif msg.startswith("PROGRESS:"):
                        return {
                            status_display: gr.update(value=f"<div>{msg[9:]}</div>"),
                            error_display: gr.update(visible=False)
                        }
                    elif msg.startswith("STOPPED:"):
                        return {
                            status_display: gr.update(value="<div style='color: orange;'>Processing stopped by user</div>"),
                            batch_status: gr.update(value="Processing stopped by user"),
                            crop_button: gr.update(interactive=True),
                            stop_button: gr.update(interactive=False),
                            error_display: gr.update(visible=False)
                        }
                    elif msg.startswith("COMPLETED:"):
                        return {
                            status_display: gr.update(value="<div style='color: green;'>Processing completed</div>"),
                            batch_status: gr.update(value=msg[10:]),
                            crop_button: gr.update(interactive=True),
                            stop_button: gr.update(interactive=False),
                            error_display: gr.update(visible=False)
                        }
                        ...
                        return {
                            status_display: gr.update(value=f"<div>{msg}</div>"),
                            error_display: gr.update(visible=False)
                        }

                def clear_status():
                    return {
                        status_display: gr.update(value="<div>Ready to start processing...</div>"),
                        batch_status: gr.update(value=""),
                        error_display: gr.update(value="", visible=False)
                    }

                def start_processing(input_folder, output_folder, aspect_ratios, yolo_folder,
                    save_yolo, batch_size, gpu_ids, overwrite, selected_class,
                    save_as_png, sam2_prompt, show_debug, skip_no_detection,
                    padding_value, padding_unit):
                    try:
                        global debug
                        debug = bool(show_debug)
                        if debug:
                            logging.critical("Debug mode enabled")
            
                        if not input_folder or not os.path.exists(input_folder):
                            return {
                                error_display: gr.update(value="Input folder does not exist", visible=True),
                                crop_button: gr.update(interactive=True),
                                stop_button: gr.update(interactive=False),
                                status_display: gr.update(value="<div>Ready to start processing...</div>")
                            }
                        ...
                        yield {
                            crop_button: gr.update(interactive=False),
                            stop_button: gr.update(interactive=True),
                            status_display: gr.update(value="<div>Initializing processing...</div>"),
                            error_display: gr.update(visible=False)
                        }

                        for status in crop_images(
                            input_folder, output_folder, aspect_ratios, yolo_folder,
                            save_yolo, batch_size, gpu_ids, overwrite, selected_class,
                            save_as_png, sam2_prompt, debug_mode=show_debug, skip_no_detection=skip_no_detection,
                            padding_value=padding_value, padding_unit=padding_unit
                        ):
                            if isinstance(status, dict):
                                yield status
                            else:
                                yield update_ui_status(status)

                        yield {
                            status_display: gr.update(value="<div style='color: green;'>Processing completed successfully</div>"),
                            batch_status: gr.update(value="Processing completed successfully"),
                            crop_button: gr.update(interactive=True),
                            stop_button: gr.update(interactive=False),
                            error_display: gr.update(visible=False)
                        }
                        ...
                        yield {
                            status_display: gr.update(value=f"<div style='color: red;'>Error occurred: {str(e)}</div>"),
                            batch_status: gr.update(value=f"Error: {str(e)}"),
                            crop_button: gr.update(interactive=True),
                            stop_button: gr.update(interactive=False),
                            error_display: gr.update(value=str(e), visible=True)
                        }

                def stop_processing_handler(progress=gr.Progress()):
                    try:
                        processing_state.stop_processing()
                        progress(0, "Stopping...")
                        yield {
                            status_display: gr.update(
                                value="""
                                <div style='color: orange;'>
                                    <h3>Processing Stop Requested</h3>
                                    <p>• Stopping all processing tasks...</p>
                                    <p>• Please wait for current operations to finish</p>
                                </div>
                                """
                            ),
                            batch_status: gr.update(
                                value="Stopping all processing tasks. Please wait..."
                            ),
                            stop_button: gr.update(interactive=False),
                            crop_button: gr.update(interactive=False)
                        }
                        ...
                        yield {
                            status_display: gr.update(
                                ...
                                <div style='color: green;'>
                                    <h3>Processing Stopped Successfully</h3>
                                    <p>• All tasks have been terminated</p>
                                    <p>• You can start a new processing job</p>
                                </div>
                                """
                            ),
                            batch_status: gr.update(
                                value="Processing has been stopped. You can start a new job."
                            ),
                            stop_button: gr.update(interactive=False),
                            crop_button: gr.update(interactive=True)
                        }
                        ...
                        yield {
                            status_display: gr.update(
                                value=f"<div style='color: red;'>Error stopping process: {str(e)}</div>"
                            ),
                            batch_status: gr.update(value=f"Error: {str(e)}"),
                            stop_button: gr.update(interactive=False),
                            crop_button: gr.update(interactive=True),
                ...
                crop_button.click(
                    fn=start_processing,
                    inputs=[
                        input_folder, output_folder, aspect_ratios,
                        yolo_folder, save_yolo, batch_size, gpu_ids,
                        overwrite_crop, selected_class, save_as_png,
                        sam2_prompt, debug_checkbox, skip_no_detection,
                        padding_value, padding_unit
                    ],
                    outputs=[
                        status_display,
            ...
            with gr.Tab("Image Resizer"):
                gr.Markdown("Resize images to various dimensions")
                resize_input_folder = gr.Textbox(label="Input Folder")
                resize_output_folder = gr.Textbox(label="Output Folder")
                resize_resolutions = gr.Textbox(label="Resolutions (comma-separated, e.g., 1024x1024,1280x720,1920x1080,...)", value="1024x1024")
                save_png_resize = gr.Checkbox(label="Save as PNG (default) or JPG", value=True)
                num_threads_resize = gr.Slider(minimum=1, maximum=os.cpu_count(), step=1, label="Number of Threads", value=4)
                overwrite_resize = gr.Checkbox(label="Overwrite existing files", value=False)
                no_crop_resize = gr.Checkbox(label="Resize without cropping (center image and add white background)", value=False)
                with gr.Row():
                    resize_button = gr.Button("Resize Images", variant="primary")
                    cancel_resize_button = gr.Button("Cancel", variant="stop")
                resize_output = gr.Textbox(label="Progress", lines=5)

                resize_button.click(resize_images, inputs=[resize_input_folder, resize_output_folder, resize_resolutions, save_png_resize, num_threads_resize, overwrite_resize, no_crop_resize], outputs=resize_output)
                cancel_resize_button.click(fn=cancel_resizer_handler, inputs=None, outputs=resize_output)

            with gr.Tab("Move Low-Res Files"):
                gr.Markdown("Move low-resolution files between folders")
                folder_a = gr.Textbox(label="Folder A (Cropped images)")
                folder_b = gr.Textbox(label="Folder B (Raw images)")
                folder_c = gr.Textbox(label="Folder C (Raw low-res images)")
                folder_d = gr.Textbox(label="Folder D (Cropped low-res images)")
                min_width = gr.Number(label="Minimum Width", value=1536)
                min_height = gr.Number(label="Minimum Height", value=1536)
                move_button = gr.Button("Move Low-Res Files")
                move_output = gr.Textbox(label="Output")

                move_button.click(move_low_res_files, inputs=[folder_a, folder_b, folder_c, folder_d, min_width, min_height], outputs=move_output)

            with gr.Tab("Rename Files"):
                gr.Markdown("Rename files in a folder")
                rename_folder = gr.Textbox(label="Folder Path")
                start_number = gr.Number(label="Starting Number", value=10001)
                rename_button = gr.Button("Rename Files")
                rename_output = gr.Textbox(label="Output")

                rename_button.click(rename_files, inputs=[rename_folder, start_number], outputs=rename_output)

            with gr.Tab("Extract Faces"):
                gr.Markdown("""
                    ## Extract Faces
                    - Extract faces/objects using YOLO object detection or SAM2 segmentation
                    - Supports the same detection methods as the Image Cropper
                    - Choose between YOLO classes or SAM2 prompts for detection
                    - Progress tracking and stop functionality
                """)
                
                with gr.Row():
                    with gr.Column(scale=1):
                        extract_input_folder = gr.Textbox(label="Input Folder")
                        extract_output_folder = gr.Textbox(label="Output Folder")
                        
                        with gr.Row():
                            padding = gr.Number(label="Padding (pixels)", value=0,
                                              info="Expand or decrease face area. Positive values expand, negative values decrease.")
                            save_png_extract = gr.Checkbox(label="Save as PNG (default) or JPG", value=True)
                        
                        with gr.Row():
                            extract_selected_class = gr.Dropdown(
                                choices=YOLO_CLASSES,
                                label="Select Detection Class (YOLO)",
                                value="person",
                                interactive=True
                            )
                            extract_sam2_prompt = gr.Textbox(
                                label="SAM2 Prompt (Leave empty to use YOLO)",
                                placeholder="e.g.: person. face. (end with periods)",
                                interactive=True
                            )
                        ...
                        with gr.Row():
                            num_threads_extract = gr.Slider(minimum=1, maximum=os.cpu_count(), step=1, label="Number of Threads", value=4)
                            extract_gpu_id = gr.Textbox(
                                label="GPU ID",
                                value="0",
                                placeholder="e.g., 0",
                                interactive=True
                            )
                            extract_debug_checkbox = gr.Checkbox(
                                label="Show Debug Logs",
                                value=False,
                                interactive=True
                            )
                    ...
                    with gr.Column(scale=1):
                        extract_status_display = gr.HTML(
                            label="Status Display",
                            value="<div>Ready to start face extraction...</div>"
                        )
                        extract_output = gr.Textbox(label="Progress", lines=8, interactive=False)
                        
                        with gr.Row():
                            extract_button = gr.Button("Extract Faces", variant="primary")
                            cancel_faces_button = gr.Button("Cancel", variant="stop")
                        
                        clear_extract_button = gr.Button("Clear Status")

                def start_face_extraction(extract_input_folder, extract_output_folder, padding, save_png_extract, 
                                        num_threads_extract, extract_selected_class, extract_sam2_prompt, 
                                        extract_gpu_id, extract_debug_checkbox):
                    try:
                        if not extract_input_folder or not os.path.exists(extract_input_folder):
                            return {
                                extract_output: "Input folder does not exist",
                                extract_status_display: gr.update(value="<div style='color: red;'>Input folder does not exist</div>")
                            }
                        
                        os.makedirs(extract_output_folder, exist_ok=True)
                        device = f"cuda:{extract_gpu_id}" if extract_gpu_id else "cuda:0"
                        
                        yield {
                            extract_status_display: gr.update(value="<div>Initializing face extraction...</div>"),
                            extract_output: "Starting face extraction...",
                            extract_button: gr.update(interactive=False),
                            cancel_faces_button: gr.update(interactive=True)
                        }
                        
                        for status in extract_faces_enhanced(
                            extract_input_folder, extract_output_folder, padding, save_png_extract,
                            num_threads_extract, extract_selected_class, extract_sam2_prompt,
                            device, extract_debug_checkbox
                        ):
                            yield {
                                extract_output: status,
                                extract_status_display: gr.update(value=f"<div>{status}</div>")
                            }
                        
                        yield {
                            extract_status_display: gr.update(value="<div style='color: green;'>Face extraction completed successfully</div>"),
                            extract_output: "Face extraction completed successfully",
                            extract_button: gr.update(interactive=True),
                            cancel_faces_button: gr.update(interactive=False)
                        }
                        
                    except Exception as e:
                        yield {
                            extract_status_display: gr.update(value=f"<div style='color: red;'>Error: {str(e)}</div>"),
                            extract_output: f"Error: {str(e)}",
                            extract_button: gr.update(interactive=True),
                            cancel_faces_button: gr.update(interactive=False)
                        }

                def stop_face_extraction():
                    try:
                        processing_state.stop_processing()
                        yield {
                            extract_status_display: gr.update(value="<div style='color: orange;'>Face extraction stopped</div>"),
                            extract_output: "Face extraction stopped by user",
                            extract_button: gr.update(interactive=True),
                            cancel_faces_button: gr.update(interactive=False)
                        }
                    except Exception as e:
                        yield {
                            extract_status_display: gr.update(value=f"<div style='color: red;'>Error stopping: {str(e)}</div>"),
                            extract_output: f"Error stopping: {str(e)}",
                            extract_button: gr.update(interactive=True),
                            cancel_faces_button: gr.update(interactive=False)
                        }

                def clear_extract_status():
                    return {
                        extract_status_display: gr.update(value="<div>Ready to start face extraction...</div>"),
                        extract_output: ""
                    }

                extract_button.click(
                    fn=start_face_extraction,
                    inputs=[
                        extract_input_folder, extract_output_folder, padding, save_png_extract,
                        num_threads_extract, extract_selected_class, extract_sam2_prompt,
                        extract_gpu_id, extract_debug_checkbox
                    ...
                    outputs=[
                        extract_status_display, extract_output, extract_button, cancel_faces_button
                    ]
                )
                
                cancel_faces_button.click(
                    fn=stop_face_extraction,
                    outputs=[
                        extract_status_display, extract_output, extract_button, cancel_faces_button
                    ]
                )
                
                clear_extract_button.click(
                    fn=clear_extract_status,
                    outputs=[
                        extract_status_display, extract_output
                    ]
                )

            with gr.Tab("Find Duplicates"):
                gr.Markdown("Find duplicate images using perceptual hashing and other methods")

                with gr.Row():
                    with gr.Column():
                        dup_source_folder = gr.Textbox(label="Source Folder")
                        dup_dest_folder = gr.Textbox(label="Destination Folder")

                        with gr.Row():
                            move_duplicates = gr.Checkbox(label="Move duplicates to destination folder", value=True)
                            hash_algorithm = gr.Dropdown(
                                choices=['ahash', 'dhash', 'phash', 'whash', 'colorhash', 'kaze', 'multi-hash'],
                                label="Hashing Algorithm",
                                value='phash'
                            )

                        default_hash_size = gr.Slider(minimum=4, maximum=32, step=1, label="Hash Size", value=16)

                        with gr.Accordion("Algorithm Details and Parameters", open=False):
                            gr.Markdown("""
                                **Algorithm Details:**

                                - **aHash (Average Hash):**
                                    - **How it works:** Calculates the average color of the image, then creates a hash based on whether each pixel is brighter or darker than the average.
                                    - **Pros:** Fast and simple. Good for detecting exact duplicates or very similar images.
                                    - **Cons:** Sensitive to small changes in color or brightness.
                                    - **Default Cutoff:** 2 (Lower = stricter, detects more duplicates)
                                    - **Example:** Imagine two identical photos, but one is slightly brighter. aHash might miss this as a duplicate if the brightness difference is significant enough.

                                - **dHash (Difference Hash):**
                                    - **How it works:** Similar to aHash but compares the difference in brightness between adjacent pixels.
                                    - **Pros:** Relatively fast, better than aHash at detecting small color changes.
                                    - **Cons:** Still can be sensitive to minor image alterations.
                                    - **Default Cutoff:** 5 (Lower = stricter)
                                    - **Example:** Two photos with slightly different color grading might be detected as duplicates by dHash.

                                - **pHash (Perceptual Hash):**
                                    - **How it works:** Uses the Discrete Cosine Transform (DCT) to focus on low-frequency components, making it robust to minor changes.
                                    - **Pros:** Very good at detecting duplicates even with compression, resizing, or minor edits.
                                    - **Cons:** Slower than aHash or dHash.
                                    - **Default Cutoff:** 8 (Lower = stricter)
                                    - **Example:** A compressed version of an original image is very likely to be identified as a duplicate by pHash.

                                - **wHash (Wavelet Hash):**
                                    - **How it works:** Uses Wavelet transform to capture image details at different scales.
                                    - **Pros:** Robust to resizing and minor color adjustments.
                                    - **Cons:** Can be slower than other hash methods.
                                    - **Default Cutoff:** 3 (Lower = stricter)
                                    - **Example:** wHash is good at finding duplicates where one image has been slightly cropped or resized.

                                - **ColorHash:**
                                    - **How it works:** Creates a hash based on the color histogram of the image.
                                    - **Pros:** Detects images with similar color distributions.
                                    - **Cons:** Sensitive to changes in image content that maintain the same color palette.
                                    - **Default Cutoff:** 50 (Higher = stricter, detects fewer duplicates)
                                    - **Example:** Two different images with a similar color scheme (e.g., both predominantly blue) might be flagged as duplicates by ColorHash.

                                - **KAZE:**
                                    - **How it works:** Detects and describes local features in images using non-linear scale spaces, making it robust to scale, rotation, and some level of viewpoint change.
                                    - **Pros:** Excellent at finding near-duplicates with significant changes in perspective, scale, or rotation. Generally more robust and accurate than SIFT/SURF.
                                    - **Cons:** Computationally more expensive and slower than hashing methods, but typically faster or comparable to SIFT/SURF. Requires the 'opencv-contrib-python' package.
                                    - **Default Matches:** 30 (Higher = stricter, detects fewer duplicates)
                                    - **Example:** KAZE is well-suited for detecting duplicates in images that have undergone various transformations like resizing, rotation, or moderate viewpoint changes.

                                - **Multi-Hash:**
                                    - **How it works:** Combines multiple hashing algorithms (aHash, dHash, pHash, wHash, ColorHash, KAZE) to improve overall accuracy.
                                    - **Pros:** Leverages the strengths of different methods, leading to better duplicate detection.
                                    - **Cons:** Can be slower as it performs multiple calculations.
                                    - **Example:** By combining the results of different algorithms, Multi-Hash can catch a wider range of duplicates that might be missed by individual methods.
                               """)
                        with gr.Row():
                            default_cutoff_ahash = gr.Slider(minimum=0, maximum=10, step=1, label="aHash Cutoff", value=5)
                            default_cutoff_dhash = gr.Slider(minimum=0, maximum=10, step=1, label="dHash Cutoff", value=5)
                            default_cutoff_phash = gr.Slider(minimum=0, maximum=10, step=1, label="pHash Cutoff", value=8)
                        with gr.Row():
                            default_cutoff_whash = gr.Slider(minimum=0, maximum=10, step=1, label="wHash Cutoff", value=3)
                            default_cutoff_colorhash = gr.Slider(minimum=0, maximum=100, step=5, label="ColorHash Cutoff", value=50)
                            default_cutoff_kaze = gr.Slider(minimum=0, maximum=100, step=5, label="KAZE Matches", value=40)
                        num_threads_duplicates = gr.Slider(minimum=1, maximum=16, step=1, label="Number of Threads", value=4)
                    with gr.Column():
                        with gr.Row():
                            find_dup_button = gr.Button("Find Duplicates", variant="primary")
                            cancel_duplicates_button = gr.Button("Cancel", variant="stop")
                        find_dup_output = gr.Textbox(label="Output", lines=5)

                def find_duplicates_wrapper(*args):
                    try:
                        dup_source_folder, dup_dest_folder, move_duplicates, hash_algorithm, default_hash_size, \
                        default_cutoff_ahash, default_cutoff_dhash, default_cutoff_phash, default_cutoff_whash, \
                        default_cutoff_colorhash, default_cutoff_kaze, num_threads_duplicates = args

                        # Validate input folders
                        if not os.path.exists(dup_source_folder):
                            return f"Source folder does not exist: {dup_source_folder}"
                        ...
                        # Always convert to list of algorithms
                        if hash_algorithm == 'multi-hash':
                            selected_algorithms = ['ahash', 'dhash', 'phash', 'whash', 'colorhash', 'kaze']
                        else:
                            selected_algorithms = [hash_algorithm]

                        # Initialize cutoff values for selected algorithms
                        cutoff = {
                            'ahash': int(default_cutoff_ahash),
                            'dhash': int(default_cutoff_dhash),
                            'phash': int(default_cutoff_phash),
                            'whash': int(default_cutoff_whash),
                            'colorhash': float(default_cutoff_colorhash),
                            'kaze': int(default_cutoff_kaze)
                        }

                        # Initialize global dictionaries for selected algorithms
                        global global_hash_dicts
                        global_hash_dicts = {alg: {} for alg in selected_algorithms}

                        # Reset duplicate images list
                        global global_duplicate_images
                        global_duplicate_images = []

                        output_text = ""
                        for message in find_duplicates_multi_hash(
                            dup_source_folder,
                            dup_dest_folder,
                            move_duplicates,
                            selected_algorithms,
                            hash_size,
                            cutoff,
                            num_threads_duplicates
                        ):
                            output_text += str(message) + "\n"

                        return output_text
                    except Exception as e:
                        logging.critical(f"Error in duplicate detection: {str(e)}")
                        return f"Error in duplicate detection: {str(e)}"

                find_dup_button.click(
                    fn=lambda *args: find_duplicates_wrapper(*args),
                    inputs=[dup_source_folder,
                        dup_dest_folder,
                        move_duplicates,
                        hash_algorithm,
                        default_hash_size,
                        default_cutoff_ahash,
                        default_cutoff_dhash,
                        default_cutoff_phash,
                        default_cutoff_whash,
                        default_cutoff_colorhash,
                        default_cutoff_kaze,
                        num_threads_duplicates
                    ],
                    outputs=find_dup_output
                )
                cancel_duplicates_button.click(fn=cancel_duplicates_handler, inputs=None, outputs=find_dup_output)

            with gr.Tab("Generate Tiled Images"):
                gr.Markdown("Generate Tiled Images: Split input images into tiled segments using a grid strategy. The grid count must be a perfect square (e.g., 4, 9, 16). Specify the desired tile resolution (for example, 1024x1024). Each tile is cropped with overlapping regions based on the original image dimensions.")
                with gr.Row():
                    with gr.Column():
                        tiled_input_folder = gr.Textbox(label="Input Folder")
                        tiled_output_folder = gr.Textbox(label="Output Folder")
                        grid_count_input = gr.Dropdown(choices=["4", "9", "16"], label="Grid Count", value="9", info="Select grid count (must be a perfect square)")
                        tile_resolution = gr.Textbox(label="Tile Resolution", value="1024x1024", placeholder="e.g., 1024x1024")
                    with gr.Column():
                        save_tiled_png = gr.Checkbox(label="Save as PNG", value=True)
                        jpeg_quality_slider = gr.Slider(minimum=1, maximum=100, step=1, label="JPEG Quality", value=90)
                        overwrite_tiled = gr.Checkbox(label="Overwrite existing files", value=False)
                        num_threads_tiled = gr.Slider(minimum=1, maximum=os.cpu_count(), step=1, label="Number of Threads", value=4)
                with gr.Row():
                    generate_tiled_button = gr.Button("Generate Tiled Images", variant="primary")
                    cancel_tiled_button = gr.Button("Cancel", variant="stop")
                tiled_output = gr.Textbox(label="Progress", lines=8)

                generate_tiled_button.click(fn=generate_tiled_images, inputs=[tiled_input_folder, tiled_output_folder, grid_count_input, tile_resolution, save_tiled_png, jpeg_quality_slider, num_threads_tiled, overwrite_tiled], outputs=tiled_output)
                cancel_tiled_button.click(fn=cancel_tiled_handler, inputs=None, outputs=tiled_output)
