...
# Configuration for CLI usage (legacy)
DEFAULT_TARGET_DIR = "index-tts/checkpoints"
DEFAULT_MODEL_CONFIGS = {
    "index_tts": {
        "repo_id": "MonsterMMORPG/Wan_GGUF",
        "subfolder": "Index_TTS2",
        "name": "Index TTS Model",
        "description": "Index TTS model from Index_TTS2 subfolder"
    }
}
...
class RobustDownloader:
    ...
    def clear_sha_cache_entry(self, repo_id: str, filename: str):
        """Clear a specific entry from the SHA256 cache to force re-fetch from HuggingFace."""
        cache_key = f"{repo_id}/{filename}"
        if cache_key in self.sha_cache:
            del self.sha_cache[cache_key]
            self.save_sha_cache()
            self.log(f"[INFO] Cleared SHA256 cache for {cache_key}")
            return True
        return False

    def refetch_sha256_from_hf(self, repo_id: str, filename: str) -> Optional[str]:
        """
        Force re-fetch SHA256 from HuggingFace, bypassing local cache.
        This is used when there's a mismatch to ensure we have the latest hash.
        """
        cache_key = f"{repo_id}/{filename}"
        ...
        self.log(f"[INFO] Re-fetching SHA256 from HuggingFace for {filename}...")
        ...
        try:
            ...
            if metadata and hasattr(metadata, 'etag'):
                ...
                if len(sha256) == 64:  # Valid SHA256 length
                    ...
                    self.save_sha_cache()
                    self.log(f"[INFO] Fresh SHA256 from HuggingFace: {sha256[:16]}...")
                    return sha256
                ...
                if file_info.rfilename == filename:
                    if hasattr(file_info, 'lfs') and file_info.lfs:
                        ...
                        if sha256:
                            self.sha_cache[cache_key] = sha256
                            self.save_sha_cache()
                            self.log(f"[INFO] Fresh SHA256 from HuggingFace API: {sha256[:16]}...")
                            return sha256

            self.log(f"[WARNING] Could not get fresh SHA256 from HuggingFace for {filename}")
            return None

        except Exception as e:
            self.log(f"[WARNING] Error re-fetching SHA256 from HuggingFace: {e}")
            return None
    ...
    def verify_file_sha256(self, filepath: str, expected_sha: str, filename: str = "", 
                           repo_id: str = "", allow_hf_refetch: bool = True) -> bool:
        """
        Verify file SHA256 hash with single-line progress.
        
        If there's a mismatch and allow_hf_refetch is True, will re-fetch the SHA256
        directly from HuggingFace to ensure we have the latest hash value.
        """
        if not expected_sha:
        ...
        try:
            ...
            if computed_sha == expected_sha:
                ...
                self.finalize_progress_line(f"[MISMATCH] SHA256 mismatch detected!")
                self.log(f"  Expected (cached): {expected_sha}")
                self.log(f"  Computed:          {computed_sha}")
                
                # On mismatch, re-fetch SHA256 directly from HuggingFace to ensure cache is current
                if allow_hf_refetch and repo_id and filename:
                    self.log(f"[INFO] Re-fetching SHA256 directly from HuggingFace to verify cache is up to date...")
                    fresh_sha = self.refetch_sha256_from_hf(repo_id, filename)
                    
                    if fresh_sha:
                        if computed_sha == fresh_sha:
                            self.log(f"[VERIFIED] SHA256 matches fresh HuggingFace hash: {computed_sha[:16]}...")
                            self.log(f"[INFO] Local cache was outdated - updated with fresh hash from HuggingFace")
                            return True
                        elif fresh_sha != expected_sha:
                            self.log(f"[INFO] HuggingFace has different SHA256: {fresh_sha[:16]}...")
                            self.log(f"[ERROR] File still doesn't match fresh HuggingFace hash - file may be corrupted")
                            self.log(f"  Fresh HF expected: {fresh_sha}")
                            self.log(f"  Computed:          {computed_sha}")
                            return False
                        else:
                            self.log(f"[ERROR] SHA256 mismatch confirmed - file is corrupted or incomplete")
                            return False
                    else:
                        self.log(f"[WARNING] Could not re-fetch SHA256 from HuggingFace")
                        self.log(f"[ERROR] SHA256 mismatch - file may be corrupted")
                        return False
                else:
                    self.log(f"[ERROR] SHA256 mismatch - file is corrupted or incomplete")
                    return False
    ...
    def download_file(self, repo_id: str, filename: str, local_dir: str) -> bool:
            ...
            if file_size:
                if actual_size == file_size:
                    ...
                    # Verify SHA256 if available and not cached
                        ...
                        if self.verify_file_sha256(filepath, expected_sha, filename, repo_id=repo_id):
                ...
                if actual_size > 1024 and expected_sha:  # > 1KB
                    ...
                    if self.is_file_verified(repo_id, filename, filepath, expected_sha):
                        ...
                        return True
                    elif self.verify_file_sha256(filepath, expected_sha, filename, repo_id=repo_id):
                        self.mark_file_verified(repo_id, filename, filepath, expected_sha)
        ...
        if success and expected_sha:
            if not self.verify_file_sha256(filepath, expected_sha, filename, repo_id=repo_id):
    ...
    def download_unknown_size(self, url: str, filepath: str, filename: str, expected_sha: str, repo_id: str = "") -> bool:
        ...
        if self.cancel_event and self.cancel_event.is_set():
            self.log(f"[CANCELLED] {filename} - download cancelled before starting")
            return False
    ...
    def download_parallel(self, url: str, filepath: str, filename: str,
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_chunks) as executor:
            ...
            futures = {}
            for chunk_id, start, end in chunks:
                # Check for cancellation before submitting each chunk
                if self.cancel_event and self.cancel_event.is_set():
                    cancelled = True
                    break
            ...
            # If cancelled during submission, cancel all pending futures
            if cancelled:
            ...
            while futures:
                ...
                if self.cancel_event and self.cancel_event.is_set():
                    self.log(f"[CANCELLING] {filename} - stopping all download threads...")
        ...
        # Handle cancellation - clean up partial files
        if cancelled:
            self.log(f"[CANCELLED] {filename} - cleaning up partial files...")
...
def download_file_with_rename(downloader: RobustDownloader, repo_id: str, remote_filename: str,
        ...
        if file_size:
            if actual_size == file_size:
                ...
                # Verify SHA256 if available and not cached
                    ...
                    if downloader.verify_file_sha256(filepath, expected_sha, local_filename, repo_id=repo_id):
            ...
            if actual_size > 1024 and expected_sha:  # > 1KB
                ...
                if downloader.is_file_verified(repo_id, remote_filename, filepath, expected_sha):
                    ...
                    return True
                elif downloader.verify_file_sha256(filepath, expected_sha, local_filename, repo_id=repo_id):
                    downloader.mark_file_verified(repo_id, remote_filename, filepath, expected_sha)
    ...
    # Verify SHA256 after successful download
    if success and expected_sha:
        if not downloader.verify_file_sha256(filepath, expected_sha, local_filename, repo_id=repo_id):
...
def download_models(
    ...
    *,
    target_dir_default: str = DEFAULT_TARGET_DIR,
    model_configs: Optional[Dict[str, Dict]] = None,
    download_config: Optional[Dict] = None,
):
    """Main download function used by CLI and external callers."""
    configs = model_configs or DEFAULT_MODEL_CONFIGS
    target_dir = download_dir if download_dir else target_dir_default
    config = download_config or DEFAULT_DOWNLOAD_CONFIG

    # Default to index_tts model
    if model_type is None:
        model_type = "index_tts"

    # Validate model type
    if model_type not in configs:
        print(f"Error: Invalid model type '{model_type}'")
        print(f"Valid options: {', '.join(DEFAULT_MODEL_CONFIGS.keys())}")
        return
    ...
    if subfolder:
        ...
        for filename in files:
            ...
            if filename.startswith(subfolder + "/"):
                # Keep the full path for filtering
    ...
    for filename in files:
        # When downloading to subfolder, remove the subfolder prefix from filename
        if subfolder and filename.startswith(subfolder + "/"):
            local_filename = filename[len(subfolder)+1:]
        else:
        ...
        if os.path.exists(filepath) and os.path.getsize(filepath) > 1024:
            ...
            if expected_sha:
                ...
                elif downloader.verify_file_sha256(filepath, expected_sha, filename, repo_id=repo_id):
    ...
    for i, filename in enumerate(to_download, 1):
        print(f"\n[{i}/{len(to_download)}]")
        # When downloading files from a subfolder, save without the subfolder prefix
        if subfolder and filename.startswith(subfolder + "/"):
            local_filename = filename[len(subfolder)+1:]
...
def download_hf_file(repo_id: str, filename: str, target_dir: str, 
    ...
    Args:
        ...
        save_filename: Optional different filename to save as (default: use original filename)
    ...
    if cancel_event and cancel_event.is_set():
        print(f"Download cancelled before starting: {filename}")
        return False
    
    try:
        success = download_file_with_rename(
            downloader=downloader,
            repo_id=repo_id,
            remote_filename=filename,
            local_dir=target_dir,
            local_filename=save_filename
        )
        return success
    except Exception as e:
...
def download_hf_snapshot(repo_id: str, target_dir: str, 
    ...
    # Check for cancellation before starting
    if cancel_event and cancel_event.is_set():
        print(f"Snapshot download cancelled before starting: {repo_id}")
        return False
    
    try:
        # For snapshot downloads, we'll download all files in the repo
        # This is a simplified implementation - you may want to add more sophisticated snapshot logic
        files = downloader.list_files(repo_id)
        ...
        for file in files:
            ...
            if cancel_event and cancel_event.is_set():
                print(f"Snapshot download cancelled during file {file}")
                return False
            
            if downloader.download_file(repo_id, file, target_dir):
                success_count += 1
        
        return success_count > 0
    except Exception as e:
        print(f"Error downloading snapshot from {repo_id}: {e}")
        return False
...
if __name__ == "__main__":
    ...
    download_models(
        args.dir,
        args.model,
        target_dir_default=DEFAULT_TARGET_DIR,
